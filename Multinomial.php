...
$GLOBALS['stemmer']  = $stemmerFactory->createStemmer();
class Multinomial
{

    var $class, $classProb, $docs, $docsc, $uniq, $term = array(), $docClass = array(), $probabily = array(), $classpn = array();
    ...
    function init()
    {
        ...
        foreach ($this->class as $c => $j) {
            ...
            foreach ($this->term as $t) {
                ...
                $this->probabily[$c][$t] = array('tf' => $jc, 'p' => (($jc + 1) / ($j + $this->uniq)));

                //echo $t . ' ((' . $jc . ' + 1) / (' . $j . ' + ' . $this->uniq . ')) = ' . (($jc + 1) / ($j + $this->uniq)) . ' <br>';
            }
            //echo '<br>';
            $this->probabily[$c]['{none}'] = array('tf' => $jc, 'p' => ((0 + 1) / ($j + $this->uniq)));
        }
    ...
    function predict($w, $prior = null, $prob = null)
    {
        ...
        if ($classnone > $classProb[array_key_first($classProb)]) {
            //echo "A";
        }
    ...
    function removeunusedchar($str)
    {
        ...
        $ready = str_replace($delimiters, " ", str_replace($delimiters, " ", $str));
        return  $ready;
    }
}
